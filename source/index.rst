.. Sublime Docs documentation master file, created by
   sphinx-quickstart on Thu Sep 30 21:07:00 2010.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Sublime Text非官方文档（中文翻译版）
=====================================

.. toctree::
   :maxdepth: 2

   关于本文档 <intro>
   安装 <getting_started/install>
   基本概念 <basic_concepts>
   编辑（尚未翻译） <editing/editing>
   搜索和替换（尚未翻译） <search_and_replace/search_and_replace_overview>
   构建系统 (批处理)（尚未翻译）<file_processing/build_systems>
   文件导航与文件管理（尚未翻译） <file_management/file_management>
   自定义内容（尚未翻译） <customization/customization>
   可扩展性与自动化（尚未翻译） <extensibility/extensibility>
   命令行（尚未翻译） <command_line/command_line>
   参考内容（尚未翻译） <reference/reference>
   词汇表（尚未翻译） <glossary/glossary>

.. Indices and tables
.. ==================

.. * :ref:`genindex`
.. * :ref:`search`

.. * :ref:`modindex`
